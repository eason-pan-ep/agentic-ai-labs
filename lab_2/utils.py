...
def get_json_from_response(json_response: str) -> json:
    """
    Get a JSON object from LLM's json formatted response
    """
    json_text = json_response.strip().removeprefix("```json").removesuffix("```").strip()
    try:
        json_obj = json.loads(json_text)
        return json_obj
    except Exception as e:
        raise ValueError("Can't parse json with wrong format")
